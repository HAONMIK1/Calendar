...
public class NoahCal extends JFrame implements ItemListener, ActionListener{
	...
	int month;
	JButton [] btn;
	public NoahCal(String string, String id) {
		super(string);
		ID=id;
		addWindowListener(
				new WindowAdapter() {
	...
	public void setDay() {
		...
		for(int day=1; day<=lastDay; day++) {
			...
			//				btn[day-1] = new JButton(String.valueOf(day)); //버튼선언해주는데 String.value 는 형변환이다. JLabel을 가운데에 입력하게둔다.
			btn[day-1] = new JButton(); //버튼선언해주는데 String.value 는 형변환이다. JLabel을 가운데에 입력하게둔다.
			
			ArrayList<CalendarBean>	lists =dao.DayMemo(ID,year,month,day);
			JLabel label = new JLabel(String.valueOf(day));
			//cb= lists.get(0);
			//cb=lists.get(1);
			if(cb!=null) {
			lbl1= new JLabel(cb.getTitle());
			lbl2= new JLabel(cb.getTitle());
			}else {
				lbl1= new JLabel(cb.getTitle());
				lbl2= new JLabel(cb.getTitle());
			}
			
			label.setVerticalAlignment(JLabel.TOP); // 텍스트를 상단에 위치하도록 설정
			...
			btn[day-1].setLayout(new BorderLayout());
			btn[day-1].add(lbl1, BorderLayout.CENTER);
			btn[day-1].add(lbl2, BorderLayout.SOUTH);
			btn[day-1].add(label, BorderLayout.NORTH); 
			btn[day-1].setFont(fnt); //버튼에 폰트를 주입한다.
			//출력하는 날짜에 대한 요일
			date.set(Calendar.DATE, day); // 19 ->1
			int w = date.get(Calendar.DAY_OF_WEEK); //요일
			if(w ==1) btn[day-1].setForeground(Color.red); //일월화수목금토 (1~7) 1은 일요일이므로 일요일에 red색깔
			if(w ==7) btn[day-1].setForeground(Color.blue); //7이므로 blue색깔

			btn[day-1].addActionListener(this);
			dayPane.add(btn[day-1]);
	...
	public void actionPerformed(ActionEvent ae) {  //액션이벤트(버튼이벤트)
		...
		for(int i=0;i<btn.length;i++) {
			if(obj==btn[i]) {
				MemoCalendar mc = new MemoCalendar(year,month,i+1,ID);
